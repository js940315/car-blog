...
SUMMARY_ACCENT = "#7dffce"   # 썸네일 강조색(민트)과 통일 — 시리즈 브랜드룩


def _emphasize_tspans(text, accent, base_fill="#ffffff"):
    """줄 안에서 *별표*로 감싼 부분을 강조색+굵게 tspan으로 만든다.
    예: "실구매가 *3천만원대*" → '3천만원대'만 민트색 800.
    별표가 없으면 전부 기본색. 홀수 인덱스가 강조 구간."""
    ...
    for i, seg in enumerate(parts):
        ...
        if i % 2 == 1:
            out.append(
                f'<tspan fill="{accent}" font-weight="800">{escape_html(seg)}</tspan>'
            )
        else:
            out.append(f'<tspan fill="{base_fill}">{escape_html(seg)}</tspan>')
    return "".join(out)
...
def build_summary_card_svg(eyebrow, title_lines, points, note="", accent=SUMMARY_ACCENT):
    ...
    points = ["짧은 한 줄", ...]. 줄 안에서 *별표*로 감싼 부분은 강조색으로 뜬다.
    예: "감가상각 폭을 *먼저* 확인", "실구매가 *3천만원대*".
    '저장해두고 싶은' 카드로 스크랩을 유도한다(스크랩·공유는 노출에 영향)."""
    p = [_card_open(accent)]
    head, y = _card_head(eyebrow, title_lines, accent)
    p.append(head)

    # 제목 아래 얇은 강조 구분선 — 위(제목)와 아래(정리) 영역을 시각적으로 나눈다
    ...
    for i, point in enumerate(points):
        ...
        cy = top + row * (i + 0.5)
        # 번호 배지: 강조색 원 + 짙은 배경색 숫자 (대비 최대)
        p.append(f'<circle cx="130" cy="{cy:.0f}" r="36" fill="{accent}"/>')
        p.append(
