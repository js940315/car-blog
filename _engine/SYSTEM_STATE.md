...
## 다음 할 일 (우선순위 — 2026-08-08 기준 갱신)
※ 아래 1~4는 **완료**됐다: repo Publish·앱권한·자산 소싱·루틴 연결(04:30 KST, 첫 정상 실행 8/4).
1. **그랜저 GN7 실사 확보** — 현재 1장이라 자동 비활성. Commons에 현행 세대가 희소하니
   `--sources wiki,openverse` 로 "Hyundai Grandeur GN7" 계열 쿼리를 더 파거나, 안 되면 프레스만 쓴다.
2. **프레스 미보유 모델 확장** — 쏘나타·스타리아·G80·G90·GV70·테슬라·수입 브랜드
   (`_engine/build_press_library.py --list` 참고).
3. **0808 4~10번은 옛 규격(6장 세트)이고 소재도 제1원칙 위반**(호주 정부 위치데이터·F1·루시드 등
