# -*- coding: utf-8 -*-
"""종목별 로고 라이브러리를 만든다 (국내완성차 / 국내부품타이어 / 해외완성차).

자동차 종목 기사는 '그 브랜드의 얼굴'이 있어야 홈판에서 인식된다.
일반 도로·차량 사진으로는 현대차 기사인지 테슬라 기사인지 알 수 없다.

    python build_ticker_library.py              # 전체
    python build_ticker_library.py --group 해외완성차
    python build_ticker_library.py --only 현대차
...
TICKERS = {
    "국내완성차": {
        "현대차": ("Hyundai Motor Company logo", "hyundai"),
        "기아": ("Kia logo 2021", "kia"),
        "제네시스": ("Genesis Motors logo", "genesis"),
        "KG모빌리티": ("KG Mobility logo", "kgm"),
    },
    "국내부품타이어": {
        "현대모비스": ("Hyundai Mobis logo", "mobis"),
        "HL만도": ("Mando Corporation logo", "mando"),
        "한온시스템": ("Hanon Systems logo", "hanon"),
        "한국타이어": ("Hankook Tire logo", "hankook"),
        "금호타이어": ("Kumho Tire logo", "kumho"),
        "넥센타이어": ("Nexen Tire logo", "nexen"),
    },
    "해외완성차": {
        "테슬라": ("Tesla Motors logo", "tesla"),
        "토요타": ("Toyota Motor Corporation logo", "toyota"),
        "폭스바겐": ("Volkswagen logo", "volkswagen"),
        "BYD": ("BYD Auto logo", "byd"),
        "포드": ("Ford Motor Company logo", "ford"),
        "GM": ("General Motors company logo", "gm"),
        "메르세데스벤츠": ("Mercedes-Benz logo", "mercedes"),
        "BMW": ("BMW logo", "bmw"),
        "스텔란티스": ("Stellantis logo", "stellantis"),
        "리비안": ("Rivian company logo", "rivian"),
        "루시드": ("Lucid Motors logo", "lucid"),
        "볼보": ("Volvo Cars logo", "volvo"),
    },
}
...
def main():
    ...
    ap.add_argument("--group", help="국내완성차 / 국내부품타이어 / 해외완성차")
    ...
    if args.group and args.group not in TICKERS:
        print("그룹은 국내완성차 / 국내부품타이어 / 해외완성차 중 하나입니다.")
        return 1
